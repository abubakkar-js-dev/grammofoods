...
class SlideshowComponent extends SliderComponent {
  constructor() {
    ...
    this.enableSliderLooping = this.slider.dataset.loop !== 'false';
    ...
    this.textAnimation = this.getAttribute('data-text-animation') || 'none';
  ...
  setAutoPlay() {
    this.autoplaySpeed = parseInt(this.slider.dataset.speed) || 5000;
    ...
    if (this.playButton) {
      this.playButton.addEventListener('click', this.autoPlayToggle.bind(this));
  ...
  onButtonClick(event) {
    ...
    const isPrev = event.currentTarget.name === 'previous';

    if (this.enableSliderLooping) {
      if (isFirstSlide && isPrev) {
        this.goToSlide(this.sliderItemsToShow.length - 1);
        return;
      }
      if (isLastSlide && !isPrev) {
        this.goToSlide(0);
        return;
      }
    }
  ...
  goToSlide(index) {
    const slideScrollPosition = this.sliderFirstItemNode.clientWidth * index;
    this.slider.scrollTo({ left: slideScrollPosition, behavior: 'smooth' });
  }
  ...
  update() {
    ...
    this.sliderDotButtons.forEach((dot, index) => {
      const isActive = index === this.currentPage - 1;
  ...
  focusOutHandling(event) {
    if (this.playButton) {
      const focusedOnPlayButton =
        event.target === this.playButton || this.playButton.contains(event.target);
      if (!this.autoplayButtonIsSetToPlay || focusedOnPlayButton) return;
      this.play();
    }
  }
  ...
  focusInHandling(event) {
    if (this.playButton) {
      const focusedOnPlayButton =
        event.target === this.playButton || this.playButton.contains(event.target);
      if (focusedOnPlayButton && this.autoplayButtonIsSetToPlay) {
        this.play();
  ...
  autoRotateSlides() {
    ...
    const slideScrollPosition = this.sliderFirstItemNode.clientWidth * nextIndex;
    this.slider.scrollTo({ left: slideScrollPosition, behavior: 'smooth' });
  }
  ...
  setSlideVisibility() {
    this.sliderItemsToShow.forEach((item, index) => {
      ...
      if (index === this.currentPage - 1) {
        linkElements.forEach((button) => button.removeAttribute('tabindex'));
        item.setAttribute('aria-hidden', 'false');
        item.removeAttribute('tabindex');
      } else {
        linkElements.forEach((button) => button.setAttribute('tabindex', '-1'));
  ...
  linkToSlide(event) {
    event.preventDefault();
    const index = this.sliderDotButtons.indexOf(event.currentTarget);
